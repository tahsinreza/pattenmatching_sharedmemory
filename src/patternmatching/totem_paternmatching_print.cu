...
PRIVATE const char* GPU_GRAPH_MEM_STR[] = {"DEVICE", "MAPPED", "MAPPED_VERTICES", "MAPPED_EDGES",
                                           "PARTITIONED_EDGES"};
...
void print_config(graph_t* graph, benchmark_options_t* options, const char* benchmark_name) {
  ...
  printf(
      "file:%s\tbenchmark:%s\tvertices:%lu\tedges:%lu\tpartitioning:%s\t"
      ...
      options->graph_file, benchmark_name, (uint64_t)graph->vertex_count,
      (uint64_t)graph->edge_count, PAR_ALGO_STR[options->par_algo], PLATFORM_STR[options->platform],
      options->alpha, options->repeat, options->gpu_count, options->thread_count,
      OMP_SCHEDULE_STR[options->omp_sched], OMP_PROC_BIND == NULL ? "false" : OMP_PROC_BIND,
      GPU_GRAPH_MEM_STR[options->gpu_graph_mem], options->gpu_par_randomized ? "true" : "false",
      options->sorted ? "true" : "false", options->edge_sort_by_degree ? "degree" : "id",
      options->edge_sort_dsc ? "dsc" : "asc", options->separate_singletons ? "true" : "false",
      options->lambda);
  fflush(stdout);
}
