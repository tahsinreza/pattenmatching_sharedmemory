...
int UniqueLabelCpu::runPatternMatching() {
  ...
  while (!finished && currentIteration < 100) {
    ...
    switch (currentStep) {
      case UniqueLabelStep::E_LCC :currentStepVertexEliminated = lccCpu.compute(*graph, &patternmatchingState);
        break;
      case UniqueLabelStep::E_CC :currentStepVertexEliminated = ccCpu.compute(*graph, &patternmatchingState);
        break;
      default:break;
    }
