...
void MultipleLabelCcBacktrackCpu<State>::init(const graph_t &graph, const graph_t &pattern) {
  patternTraversalVector.resize(pattern.vertex_count);
}

template<class State>
bool MultipleLabelCcBacktrackCpu<State>::isInConstraintVector(const MultipleLabelCircularConstraint &constraint) const {
...
template<class State>
__host__ void MultipleLabelCcBacktrackCpu<State>::buildConstraintList(
...
template<class State>
__host__ error_t
MultipleLabelCcBacktrackCpu<State>::preprocessPatern(const graph_t &pattern) {
  // for loop
  std::vector<vid_t> historyVertexId;
  std::vector<weight_t> historyVertexLabel;
...
template<class State>
__host__ void MultipleLabelCcBacktrackCpu<State>::printCircularConstraint(std::ostream &ostream) const {
...
template<class State>
bool MultipleLabelCcBacktrackCpu<State>::checkConstraint(
    const graph_t &graph,
    State *globalState,
...
template<class State>
__host__ void MultipleLabelCcBacktrackCpu<State>::resetState(State *globalState) {
...
template<class State>
int MultipleLabelCcBacktrackCpu<State>::getCircularConstraintNumber() const {
  return circularConstraintVector.size();
}

template<class State>
const std::vector<MultipleLabelCircularConstraint> &MultipleLabelCcBacktrackCpu<State>::getCircularConstraintVector() const {
  return circularConstraintVector;
}

template<class State>
void MultipleLabelCcBacktrackCpu<State>::setCircularConstraintIterator(
