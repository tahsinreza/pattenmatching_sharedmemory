...
PRIVATE inline __attribute__((always_inline)) void bfs_cpu_process_vertex(graph_t* subgraph,
                                                                          bfs_state_t* state,
                                                                          vid_t v, int pid,
                                                                          bool& finished) {
...
PRIVATE void bfs_cpu_sparse_frontier(graph_t* subgraph, bfs_state_t* state, int pid) {
  ...
  bool finished = true;
  vid_t count = 0;
#pragma omp parallel for schedule(runtime) reduction(& : finished) reduction(+ : count)
...
PRIVATE void bfs_cpu_dense_frontier(graph_t* subgraph, bfs_state_t* state, int pid) {
  bool finished = true;
  vid_t count = 0;
#pragma omp parallel for schedule(runtime) reduction(& : finished) reduction(+ : count)
...
void bfs_cpu(partition_t* par, bfs_state_t* state) {
  ...
  if (engine_partition_algorithm() == PAR_SORTED_ASC) {
    ...
    if (frontier_count > 0) {
      if (frontier_count <= (subgraph->vertex_count * TRV_FRONTIER_SPARSE_THRESHOLD)) {
...
__global__ void bfs_kernel(partition_t par, bfs_state_t state, const vid_t* __restrict frontier,
                           vid_t count) {
  if (THREAD_GLOBAL_INDEX >= vwarp_thread_count(count, VWARP_WIDTH, VWARP_BATCH)) return;
  ...
  vid_t end_vertex = start_vertex + vwarp_warp_batch_size(count, VWARP_WIDTH, VWARP_BATCH);
  ...
  for (vid_t i = start_vertex; i < end_vertex; i++) {
    ...
    if (USE_FRONTIER || (cost[v] == state.level)) {
      ...
      if (v >= par.subgraph.vertex_ext) {
        nbrs = par.subgraph.edges_ext + (vertices[v] - par.subgraph.edge_count_ext);
      }
...
#endif /* FEATURE_SM35  */
    void
    bfs_launch_gpu(partition_t* par, bfs_state_t* state, vid_t* frontier, vid_t vertex_count,
                   cudaStream_t stream) {
  if (!USE_FRONTIER) vertex_count = par->subgraph.vertex_count;
  const int threads = MAX_THREADS_PER_BLOCK;
  dim3 blocks;
  kernel_configure(vwarp_thread_count(vertex_count, VWARP_WIDTH, VWARP_BATCH), blocks, threads);
  bfs_kernel<VWARP_WIDTH, VWARP_BATCH, USE_FRONTIER>
      <<<blocks, threads, 0, stream>>>(*par, *state, frontier, vertex_count);
}

typedef void (*bfs_gpu_func_t)(partition_t*, bfs_state_t*, vid_t*, vid_t, cudaStream_t);

#ifdef FEATURE_SM35
PRIVATE __global__ void bfs_launch_at_boundary_kernel(partition_t par, bfs_state_t state) {
...
PRIVATE void bfs_tuned_launch_gpu(partition_t* par, bfs_state_t* state) {
  ...
  if (engine_partition_algorithm() == PAR_SORTED_ASC) {
    ...
    // pre-allocated space
    frontier_update_bitmap_gpu(&state->frontier, state->visited[par->id], par->streams[1]);
  ...
  if (engine_sorted() && use_frontier) {
    ...
    // advantage of that
    frontier_update_list_gpu(&state->frontier, state->level, state->cost, par->streams[1]);
    frontier_update_boundaries_gpu(&state->frontier, &par->subgraph, par->streams[1]);
  ...
  BFS_GPU_FUNC[par_alg][use_frontier](par, state, state->frontier.list, vertex_count,
                                      par->streams[1]);
  CALL_CU_SAFE(cudaGetLastError());
}
...
PRIVATE void bfs_gpu(partition_t* par, bfs_state_t* state) {
  ...
  for (int pid = 0; pid < engine_partition_count(); pid++) {
    if ((pid == par->id) || (par->outbox[pid].count == 0)) continue;
    bitmap_copy_gpu(state->visited[pid], (bitmap_t)par->outbox[pid].push_values,
...
PRIVATE inline void bfs_scatter_cpu(grooves_box_table_t* inbox, bfs_state_t* state,
                                    bitmap_t visited) {
  bitmap_t remotely_visited = (bitmap_t)inbox->push_values;
#pragma omp parallel for schedule(runtime)
  for (vid_t word_index = 0; word_index < bitmap_bits_to_words(inbox->count); word_index++) {
...
__global__ void bfs_scatter_kernel(const bitmap_t __restrict rmt_visited,
                                   const vid_t* __restrict rmt_nbrs, vid_t word_count,
                                   bitmap_t visited, cost_t* cost, cost_t level) {
  if (THREAD_GLOBAL_INDEX >= vwarp_thread_count(word_count, VWARP_WIDTH, BATCH_SIZE)) return;
  ...
  vid_t end_word =
      start_word + vwarp_warp_batch_size(word_count, VWARP_WIDTH, BATCH_SIZE, THREADS_PER_BLOCK);
  int warp_offset = vwarp_thread_index(VWARP_WIDTH);
...
PRIVATE void bfs_scatter(partition_t* par) {
  ...
  for (int rmt_pid = 0; rmt_pid < engine_partition_count(); rmt_pid++) {
    ...
    if (par->processor.type == PROCESSOR_CPU) {
      ...
      kernel_configure(vwarp_thread_count(word_count, warp_size, batch_size), blocks, threads);
      bfs_scatter_kernel<warp_size, batch_size, threads><<<blocks, threads, 0, par->streams[1]>>>(
          (bitmap_t)inbox->push_values, inbox->rmt_nbrs, word_count, state->visited[par->id],
          state->cost, state->level);
      CALL_CU_SAFE(cudaGetLastError());
    } else {
      assert(false);
    }
...
PRIVATE void bfs_aggregate(partition_t* par) {
  ...
  if (par->processor.type == PROCESSOR_CPU) {
    ...
    CALL_CU_SAFE(cudaMemcpy(state_g.cost_h, state->cost, subgraph->vertex_count * sizeof(cost_t),
                            cudaMemcpyDefault));
...
PRIVATE inline void bfs_init_gpu(partition_t* par) {
  ...
  for (int pid = 0; pid < engine_partition_count(); pid++) {
    if (pid != par->id && par->outbox[pid].count != 0) {
      ...
      bitmap_reset_gpu((bitmap_t)par->outbox[pid].push_values, par->outbox[pid].count,
                       par->streams[1]);
    }
  }
  // set the source vertex as visited
  if (GET_PARTITION_ID(state_g.src) == par->id) {
    bfs_init_kernel<<<1, 1, 0, par->streams[1]>>>(state->visited[par->id],
...
PRIVATE inline void bfs_init_cpu(partition_t* par) {
  ...
  for (int pid = 0; pid < engine_partition_count(); pid++) {
    if (pid != par->id && par->outbox[pid].count != 0) {
      ...
      bitmap_reset_cpu((bitmap_t)par->outbox[pid].push_values, par->outbox[pid].count);
    }
  }
  // set the source vertex as visited
  if (GET_PARTITION_ID(state_g.src) == par->id) {
...
PRIVATE void bfs_init(partition_t* par) {
  ...
  CALL_SAFE(
      totem_malloc(par->subgraph.vertex_count * sizeof(cost_t), type, (void**)&(state->cost)));
  totem_memset(state->cost, INF_COST, par->subgraph.vertex_count, type, par->streams[1]);
  if (GET_PARTITION_ID(state_g.src) == par->id) {
    // For the source vertex, initialize cost.
    totem_memset(&((state->cost)[GET_VERTEX_ID(state_g.src)]), (cost_t)0, 1, type, par->streams[1]);
  }
...
error_t bfs_hybrid(vid_t src, cost_t* cost) {
  ...
  engine_config_t config = {NULL,     bfs,          bfs_scatter,   NULL,
                            bfs_init, bfs_finalize, bfs_aggregate, GROOVES_PUSH};
  engine_config(&config);
  if (engine_largest_gpu_partition()) {
    CALL_SAFE(totem_malloc(engine_largest_gpu_partition() * sizeof(cost_t), TOTEM_MEM_HOST,
                           (void**)&state_g.cost_h));
  }
  engine_execute();
