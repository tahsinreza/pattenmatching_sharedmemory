...
PRIVATE void display_help(char* exe_name, int exit_err)
{
    printf("Usage: %s [options] graph_file\n"
           ...
           exe_name, BENCHMARK_BFS, get_gpu_count(), PAR_RANDOM, PAR_SORTED_ASC, PAR_SORTED_DSC, GPU_GRAPH_MEM_DEVICE,
           GPU_GRAPH_MEM_MAPPED, GPU_GRAPH_MEM_MAPPED_VERTICES, GPU_GRAPH_MEM_MAPPED_EDGES,
           GPU_GRAPH_MEM_PARTITIONED_EDGES, PLATFORM_CPU, PLATFORM_GPU, PLATFORM_HYBRID, REPEAT_MAX, omp_sched_static,
           omp_sched_dynamic, omp_sched_guided, omp_get_max_threads(), omp_get_max_threads());
    exit(exit_err);
}
...
{
    optarg = NULL;
    int ch, platform, par_algo, gpu_graph_mem;
    while(((ch = getopt(argc, argv, "a:b:cdeg:i:l:m:op:qr:s:t:h")) != EOF)) {
        ...
        case 's':
            ...
            if(options.omp_sched != omp_sched_static && options.omp_sched != omp_sched_dynamic &&
